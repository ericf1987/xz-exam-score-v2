...
@Function(description = "学校成绩-三率对比", parameters = {
    ...
    private Map<String,Object> getSchoolAverageMap(String projectId, String schoolId, String subjectId) {
        ...
        for(Document projectDoc : projectList){
            if(null != projectDoc && !projectDoc.isEmpty()){
                ...
                List<Document> scoreLevels = scoreLevelService.getScoreLevelRate(projectId, Range.school(schoolId), Target.subject(subjectId));
                average.put("projectName", projectName);
                average.put("startDate", startDate);
                average.put("scoreLevels", scoreLevels);
        ...
        for (Document classDoc : classList){
            ...
            for(Document projectDoc : projectList){
                if(null != projectDoc && !projectDoc.isEmpty()) {
                    ...
                    List<Document> scoreLevels = scoreLevelService.getScoreLevelRate(projectId, Range.clazz(classId), Target.subject(subjectId));
                    average.put("projectName", projectName);
                    average.put("startDate", startDate);
                    average.put("scoreLevels", scoreLevels);
                    averages.add(average);
